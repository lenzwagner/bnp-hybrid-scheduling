...
class InstanceGenerator:
    ...
    def generate_all_instances(self):
        ...
        print(f"  - Target instances per (T, D_focus, pttr): {target_seeds_per_combination}")
        ...
        for pttr, T, D_focus in product(self.pttr_variants, self.T_variants, self.D_focus_variants):
            ...
            if current_count < target_seeds_per_combination:
                ...
                combos_to_refill.append((combo_key, missing))

        if combos_to_refill:
            print(f"\n{len(combos_to_refill)} combination(s) need refilling:")
            for (pttr, T, D_focus), missing in combos_to_refill:
                print(f"  - pttr={pttr}, T={T}, D_focus={D_focus}: needs {missing} more instance(s)")
            ...
            for (pttr, T, D_focus), missing in combos_to_refill:
                ...
                combo_key = (pttr, T, D_focus)
                print(f"\nRefilling pttr={pttr}, T={T}, D_focus={D_focus} (need {missing} more):")
                ...
                while successful_per_combo[combo_key] < target_seeds_per_combination and attempts < max_attempts_per_combo:
                    ...
                    instance_data = self.generate_instance(current_seed, pttr, T, D_focus)
