...
def solve_instance(seed, D_focus, pttr='medium', T=2, allow_gaps=False, use_warmstart=True, dual_smoothing_alpha=None, learn_type=0, app_data_overrides=None, T_demand=None, pre_generated_data=None, severity_mix=None):
    """
    ...
    instance_data = {
        ...
        'T': len(cg_solver.T),
        'D': len(cg_solver.D),
        'severity_mix': severity_mix,  # Store severity mix configuration
...
def main_loop():
    """
    Main loop to solve instances from Excel file.
    ...
    # ===========================
    import argparse
    parser = argparse.ArgumentParser(description='Run Column Generation with learning')
    parser.add_argument('--scenarios', type=str, default=None,
                        help='Path to Excel file with scenarios')
    ...
    if args.scenarios:
        # Use provided scenarios file
        newest_excel = args.scenarios
    else:
        # Find the newest Excel file in instances directory
        instances_dir = 'instances'
        if not os.path.exists(instances_dir):
            raise FileNotFoundError(f"Instances directory not found: {instances_dir}")
        
        excel_files = [f for f in os.listdir(instances_dir) if f.endswith('.xlsx')]
        
        if not excel_files:
            raise FileNotFoundError(f"No Excel files found in {instances_dir}")
        
        # Get the newest file by modification time
        excel_files_full = [os.path.join(instances_dir, f) for f in excel_files]
        newest_excel = max(excel_files_full, key=os.path.getmtime)
    ...
    for idx, row in scenarios_df.iterrows():
        ...
        # Read severity_mix if available in the Excel file
        severity_mix = None
