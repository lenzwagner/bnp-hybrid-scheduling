...
def solve_instance(seed, D_focus, pttr='medium', T=2, allow_gaps=False, use_warmstart=True, dual_smoothing_alpha=None, learn_type=0):
    ...
    MEMORY-OPTIMIZED VERSION: Explicitly cleans up resources after solving.
    """
    ...
    # Parallelization settings (REDUCED FOR MEMORY EFFICIENCY)
    use_parallel_pricing = True
    use_parallel_tree = False  # Disable parallel tree to reduce memory
    import os
    # Reduce number of workers to save memory
    n_pricing_workers = min(os.cpu_count() // 2, 2) if use_parallel_pricing else 1
    n_tree_workers = 1  # Keep at 1 to minimize memory usage
    ...
    labeling_spec = {
        ...
        # Tree exploration parallelization (DISABLED)
    ...
    try:
        ...
        # Solve with 30-minute timeout per instance
        # REDUCED MAX_NODES to save memory
        results = bnp_solver.solve(time_limit=1800, max_nodes=200)  # Reduced from 300 to 200
        ...
        # ===========================
        # SESSION TRANSITION MATRIX (DISABLED TO SAVE MEMORY)
        # ===========================
        # Transition matrix computation is disabled to reduce memory usage
        
        return instance_data
        
    except Exception as e:
        logger.error(f"Error solving instance seed={seed}, D_focus={D_focus}: {str(e)}", exc_info=True)
        raise
        ...
        # 1. Clean up BnP solver
        if bnp_solver is not None:
            try:
                # Clean up pricing pool
                if hasattr(bnp_solver, '_cleanup_pricing_pool'):
                    bnp_solver._cleanup_pricing_pool()
                
                # Clean up node data
                if hasattr(bnp_solver, 'nodes'):
                    for node in bnp_solver.nodes.values():
                        if hasattr(node, 'master') and node.master is not None:
                            try:
                                node.master.model.dispose()
                            except:
                                pass
                    bnp_solver.nodes.clear()
                
                del bnp_solver
            except Exception as e:
                logger.warning(f"Error cleaning up BnP solver: {e}")
        
        # 2. Clean up CG solver
        if cg_solver is not None:
            try:
                # Dispose Gurobi models
                if hasattr(cg_solver, 'master') and cg_solver.master is not None:
                    if hasattr(cg_solver.master, 'model'):
                        try:
                            cg_solver.master.model.dispose()
                        except:
                            pass
                
                if hasattr(cg_solver, 'compact') and cg_solver.compact is not None:
                    if hasattr(cg_solver.compact, 'model'):
                        try:
                            cg_solver.compact.model.dispose()
                        except:
                            pass
                
                # Clean up subproblems
                if hasattr(cg_solver, 'sub'):
                    for sp in cg_solver.sub.values():
                        if hasattr(sp, 'model'):
                            try:
                                sp.model.dispose()
                            except:
                ...
                if hasattr(cg_solver, 'global_solutions'):
                    cg_solver.global_solutions.clear()
                
                del cg_solver
            except Exception as e:
                logger.warning(f"Error cleaning up CG solver: {e}")
...
def main_loop():
    ...
    MEMORY-OPTIMIZED VERSION with incremental saving.
    """
    ...
    excel_filename = f"results/cg/results_loop_{timestamp}.xlsx"
    pickle_filename = f"results/cg/results_dict_{timestamp}.pkl"
    ...
    for idx, row in scenarios_df.iterrows():
        ...
        for lt in learn_types:
            ...
            try:
                ...
                # INCREMENTAL SAVE after each successful instance
                try:
                    ...
                    print(f"  ⚠ Warning: Could not save incrementally: {save_error}")
    ...
    try:
        results_df.to_excel(excel_filename, index=False)
        print(f"\n✓ Final results saved to {excel_filename}")
    except Exception as e:
        print(f"\n✗ Could not save final Excel: {e}")
