...
def calculate_extra_metrics(cg_solver, inc_sol, patients_list, derived_data, T, start_day=None, end_day=None):
    ...
    # ===========================
    try:
        from calculate_transition_matrix import compute_session_transition_matrix
        
        # Build app_data from cg_solver attributes
        app_data = {
            'learn_type': [cg_solver.learn_type] if hasattr(cg_solver, 'learn_type') else ['sigmoid'],
            'theta_base': [cg_solver.theta_base] if hasattr(cg_solver, 'theta_base') else [0.3],
            'k_learn': [cg_solver.k_learn] if hasattr(cg_solver, 'k_learn') else [0.3],
            'infl_point': [cg_solver.infl_point] if hasattr(cg_solver, 'infl_point') else [5],
            'lin_increase': [0.05]
        }
        
        # Compute transition matrix with automatic Y-based bins
        transition_probs, transition_df, transition_counts = compute_session_transition_matrix(
            cg_solver=cg_solver,
            inc_sol=inc_sol,
            app_data=app_data,
            theta_bins=None,  # Automatic Y-based bins
            patients_list=patients_list
        )
        
        # Store transition matrix data in metrics dict
        metrics['transition_probabilities'] = transition_probs
        metrics['transition_counts'] = transition_counts
        
        # Store flattened metrics for Excel export
        for _, row in transition_df.iterrows():
            # Clean up theta_range for valid dict key
            theta_range = (row['Theta_Range']
                          .replace('[', '').replace(')', '').replace(',', '_')
                          .replace(' ', '').replace('θ=', 'th').replace('-', '_')
                          .replace('=', ''))
            from_session = row['From_Session']
            key_prefix = f"trans_{theta_range}_{from_session}"
            metrics[f"{key_prefix}_to_Human"] = row['To_Human']
            metrics[f"{key_prefix}_to_AI"] = row['To_AI']
            metrics[f"{key_prefix}_to_Gap"] = row['To_Gap']
            metrics[f"{key_prefix}_count"] = row['Total_Transitions']
        
    except Exception as e:
        print(f"Warning: Could not compute transition matrix: {str(e)}")
        metrics['transition_matrix_error'] = str(e)
